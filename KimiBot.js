...
function parseArgs() {
  ...
  for (const a of argv) {
    ...
    else if (a === '--block-media') cfg.blockMedia = true;
...
async function setupPageEvasion(page, profile, cfg) {
  ...
  // Request interception - blocking tracking/verification only (no media blocking)
  ...
  page.on('request', (req) => {
    ...
    // Media blocking removed to preserve engagement
...
function appendCSV(row, cfg) {
  try {
    ...
    const headers = 'timestamp,run,tab,referrer_clicked,target_final,post_opened,post_final,duration_ms,proxy_used,referrer_used,pages_visited,bounce,return_visitor\n';
  ...
  while (!stop && (cfg.forever || run < cfg.runs)) {
    ...
    try {
      ...
      const executions = browsers.map(async ({ browser, flow }) => {
        const start = Date.now();
        const engagement = { scrollEvents: 0, mouseBursts: 0, startTime: Date.now() };
      ...
      for (const { flow } of browsers) {
        ...
        log('success', `Tab ${flow.tab}: pages=${flow.results.pagesVisited}, bounced=${flow.bounceFromHomepage}, return=${flow.isReturn}, proxy=${flow.results.proxyUsed ? 'yes' : 'no'}`);
      }
